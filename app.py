...
# Page Configuration
st.set_page_config(
    page_title="Sessiz Akım — Quiet Current Portal",
    page_icon="⚓",
    layout="wide",
...
max_diesel_lph = 30.0
cruise_diesel_lph = max_diesel_lph * (
    (cruise_speed / 10.0) ** 3
)  # Küp Kuralı ile Dinamik Tüketim
...
new_degradation = (
    (bat_cost_tl / 3000) * (brut_kwh / spec["batCapacity"]) * operating_days
)
...
old_co2 = (
    spec["merged"] * cruise_diesel_lph * cruise_hours * operating_days * 2.68
) / 1000
...
with tech_col2:
  st.info(
      f"**{cruise_speed:.1f} Knots Seyir Gücü:**\n\n{cruise_power_kw:.1f} kW"
      f" (Dizel: {cruise_diesel_lph:.2f} L/h)"
  )
with tech_col3:
  st.info(
